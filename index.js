...
//FIM DOS ARQUIVOS ANTIS

//TIC-TAC-TOE By: Resen
...
client.on('CB:action,,battery', json => {
	...
	╰───────────────╯						
				...
				only: {
					...
					admin: '❌ Este comando só pode ser usado pelos adms de grupo! ❌',
...
if (budy.includes("wa.me")){
		     if (!isGroup) return
		     if (!isAntiLink) return
		     if (isGroupAdmins) return reply(`*${pushname}* vc é adm por isso não vou te banir!`)
		    client.updatePresence(from, Presence.composing)
		   var Kick = `${sender.split("@")[0]}@s.whatsapp.net`
		    setTimeout( () => {
	    	reply(`*𝑒𝑙𝑖𝑚𝑖𝑛𝑎𝑑𝑜 𝑑𝑜 𝑔𝑟𝑢𝑝𝑜*`)
...
//CONFIGURAÇÕES STICKER NAME 2
			function addMetadata(packname, author) {	
				if (!packname) packname = 'Gekyume'; if (!author) author = 'JoãoNetoFM';	
...
susp = `*@${mentioned[0].split('@')[0]} Levou Aquele Banzinho e Não Poderá Usar Os Comandos!*`
...
fs.writeFileSync('./database/banned.json', JSON.stringify(ban))
susp = `*@${mentioned[0].split('@')[0]} foi desbloqueado Bem Vindo De Volta! :D*`
mentions(`${susp}`, mentioned, true)   
break
case 'addvip':
if (!isRegistrar) return reply(mess.registro)//PEDI O REGISTRO
...
fs.writeFileSync('./database/vip.json', JSON.stringify(premium))
vip = `×@${mentioned[0].split('@')[0]} Você virou Vip no Gekyume Bot!×`
...
if (!isRegistrar) return reply(mess.registro)//PEDI O REGISTRO
                    if (!isPremium) return reply(mess.only.premium)
                    client.sendMessage(from, pack18(prefix) , text, {quoted: mek, quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg","caption": "Gekyume ", 'jpegThumbnail': fs.readFileSync('kk/sticker/botlogo.webp')}}}})					
                break
case 'utils':
if (!isRegistrar) return reply(mess.registro)//PEDI O REGISTRO
...
if (!isRegistrar) return reply(mess.registro)//PEDI O REGISTRO
					...
					if (Number(args[0]) === 1) {
						if (isAntiFake) return reply('Já esta ativo')
						antifake.push(from)
						fs.writeFileSync('./src/antifake.json', JSON.stringify(antifake))
						reply('Recurso Ant-Fake Ativado✔️')
					} else if (Number(args[0]) === 0) {
						antifake.splice(from, 1)
						fs.writeFileSync('./src/antifake.json', JSON.stringify(antifake))
						reply('Recurso Ant-Fake Desativado✔️')
					} else {
						reply('1 para ativar, 0 para desativar')
					}
					} catch {
				...
				if (!isRegistrar) return reply(mess.registro)//PEDI O REGISTRO
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* João NetoF.M.\n*WPP:* wa.me/+556492878165\n\n\nEspero que tenham gostado do bot 🥵'})
					  client.sendMessage(from, 'Ctt do meu dono ai Wa.me/556492878165 , pfv n flode',MessageType.text, { quoted: freply} )
                    break
...
break
				...
				if (!isRegistrar) return reply(mess.registro)//PEDI O REGISTRO
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `[ Transmissão Gekyume ]\n\n${body.slice(4)}`})
						}
						reply('Transmissao enviada')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `[ Transmissão Gekyume ]\n\n${body.slice(4)}`)
						}
...
if (!isRegistrar) return reply(mess.registro)//PEDI O REGISTRO
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						buff = await client.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							client.sendMessage(_.jid, buff, image, {caption: `[ Aviso! Gekyume Bot ]\n\n${body.slice(4)}`})
						}
						reply('Transmissao enviada')
					} else {
						for (let _ of anu) {
							sendMess(_.jid, `[ Aviso! Gekyume Bot ]\n\n${body.slice(7)}`)
						}
						reply('Tm enviada com sucesso')
					}
					break
...
if (!isRegistrar) return reply(mess.registro)//PEDI O REGISTRO
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ Gekyume significa: “um estado alterado” ]\n\n${body.slice(10)}`)
						}
						reply('Tm enviada com sucesso')
					}
					break
...
if (!isRegistrar) return reply(mess.registro)//PEDI O REGISTRO
					...
					rate = body.slice(5)
					var ti =['4','9','17','28','34','48','59','62','74','83','97','100','300','94','75','82','41','39']
			     ...
			     if (!isRegistrar) return reply(mess.registro)//PEDI O REGISTRO
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('end', function () {
								console.log('Finish')
								exec(`webpmux -set exif ${addMetadata('Jekyume', authorname)} ${ran} -o ${ran}`, async (error) => {
						...
						await removeBackgroundFromImageFile({path: media, apiKey: keyrmbg, size: 'auto', type: 'auto', ranp}).then(res => {
							...
							exec(`ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${ranw}`, (err) => {
								...
								exec(`webpmux -set exif ${addMetadata('Jekyume', authorname)} ${ranw} -o ${ranw}`, async (error) => {
									if (error) return reply(mess.error.stick)
...
reply("ok, chefe esse mano perdeu o adm!")
break
...
reply("ok, Chefe esse mano agora é adm!")
break
///------------
				case 'add':
				if (!isRegistrar) return reply(mess.registro)//PEDI O REGISTRO
...
if ((budy === `vai tmnc`)){                     
                   ...
                   client.sendMessage(from, 'Disse A Putinha...', text, {quoted: freply})
					...
					if ((budy === `bot arrombado`)){
						client.sendMessage(from, 'Meu Pau Que Te Deixa Gelado', text,{quoted: freply})
					}
					if ((budy === `Bot arrombado`)){
						client.sendMessage(from, 'Meu Pau Que Te Deixa Gelado', text,{quoted: freply})
					}
					//Fdp Filha da puta
					if ((budy === `Bot filha da puta`)){
						client.sendMessage(from, 'Meu Pau Que Te Cutuca :D', text,{quoted: freply})
					}
					if ((budy === `bot filha da puta`)){
						client.sendMessage(from, 'Meu Pau Que Te Cutuca :D', text,{quoted: freply})
					}
					...
					//REVOLTADO
					if ((budy === `Bot revoltado`)){
						client.sendMessage(from, 'Meu Pau No teu Rabo. kakaka', text,{quoted: freply})
					}
					if ((budy === `bot revoltado`)){
						client.sendMessage(from, 'Meu Pau No teu Rabo. kakaka', text,{quoted: freply})
					}
					...
					if ((budy === `BOT REVOLTADO`)){
						client.sendMessage(from, 'Vai se fude menó', text,{quoted: freply})
					}if ((budy === `bot do krl`)){
						client.sendMessage(from, 'Vai se fude menó', text,{quoted: freply})
					}if ((budy === `Bot do krl`)){
						client.sendMessage(from, 'Vai se fude menó', text,{quoted: freply})
					...
					//Reply Gekyume
					if ((budy === `Gekyume`)){
						client.sendMessage(from, 'Eita, Ele/a Sabe Meu Nome', text,{quoted: freply})
					}
					if ((budy === `gekyume`)){
						client.sendMessage(from, 'Eita, Ele/a Sabe Meu Nome', text,{quoted: freply})
					}
					if ((budy === `Vlw Bot`)){
						client.sendMessage(from, 'É noix bro tmj.', text,{quoted: freply})
					}
					if ((budy === `Vlw bot`)){
						client.sendMessage(from, 'É noix bro tmj.', text,{quoted: freply})
					}
					if ((budy === `obrigada bot`)){
						client.sendMessage(from, 'É noix bro tmj.', text,{quoted: freply})
					}
					if ((budy === `Obrigada bot`)){
						client.sendMessage(from, 'É noix bro tmj.', text,{quoted: freply})
...
//Reply Bot On?
if ((budy === `Bot ta on?`)){                     
                   client.sendMessage(from, 'será que eu tou?', text, {quoted: freply})
                    }
if ((budy === `Bot on?`)){                     
                   client.sendMessage(from, 'será que eu tou?', text, {quoted: freply})
                    }
...
//Reply Oi
if ((budy === `Oi`)){                     
                   client.sendMessage(from, 'Oi coisa linda <3', text, {quoted: freply})
                    }
